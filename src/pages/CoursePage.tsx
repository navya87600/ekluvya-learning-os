...
import { chapters, type Chapter } from "@/data/mockData";
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden mb-4 hover:shadow-ek transition-shadow">
      ...
      >
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-heading font-extrabold text-sm flex-shrink-0 ${
          isCompleted ? "bg-ek-green text-primary-foreground" :
          isInProgress ? "bg-gradient-to-br from-brand to-brand-2 text-primary-foreground" :
        ...
        <div className="flex items-center gap-3">
          {chapter.progress > 0 && (
            <div className="w-24 h-[5px] bg-surface-2 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-brand to-brand-2 rounded-full transition-all" style={{ width: `${chapter.progress}%` }} />
            </div>
          )}
          <motion.span
        ...
        {open && (
          ...
          >
            <div className={`px-5 pb-5 border-t pt-4 ${
              inProgressTopics > 0 ? "border-brand/20 bg-brand/[0.02]" : "border-border"
            }`}>
              {/* Chapter description */}
              <p className="text-[13px] text-ink-50 leading-relaxed mb-4">
                {chapter.topics.map(t => t.name).join(" · ")}
              </p>

              {/* Video providers */}
              <div className="font-mono text-[10px] font-bold text-ai-1 tracking-[1.5px] uppercase mb-3">
                📹 {chapter.topics[0]?.videos.length || 4} Provider Videos — Choose Your Teaching Style
              </div>
              <div className="grid grid-cols-3 gap-2.5 mb-4">
                {(chapter.topics[0]?.videos || []).slice(0, 3).map((v) => (
                  <div key={v.id} className={`rounded-lg overflow-hidden transition-all hover:-translate-y-0.5 ${
                    ...
                    <div className="p-2">
                      <div className={`text-[10px] font-bold font-mono mb-0.5 ${
                        v.isRecommended ? "text-brand" : v.provider.includes("KHAN") ? "text-ek-green" : "text-ek-blue"
                      }`}>
              ...
              {chapter.progress > 0 && (
                <div className="flex items-center gap-2 pt-3 border-t border-surface-2">
                  <span className="text-[11px] font-mono font-semibold text-ai-1">🤖 AI: {chapter.progress}% mastery</span>
              ...
              <div className="flex items-center gap-2 mt-3">
                {isCompleted && (
                  <span className="text-[10px] font-bold font-mono text-ek-green">✓ Complete</span>
                )}
                {inProgressTopics > 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold font-mono tracking-[0.5px] uppercase bg-ai-1/[0.12] text-ai-1 border border-ai-1/25">
                    ▶ Watching
                  </span>
                )}
              </div>
    ...
    <AppLayout title="Courses">
      ...
      <div className="bg-card border border-border rounded-lg p-4 mb-6 flex items-center gap-6">
        ...
        <div className="flex-1 h-2 bg-surface-2 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-brand to-brand-2 rounded-full" style={{ width: "26%" }} />
        </div>
